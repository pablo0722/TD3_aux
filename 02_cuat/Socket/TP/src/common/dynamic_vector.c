...
t_key dynamic_vector_add(t_dynamic_vector *dyn_vec, void *element)
{
	...
	ret = last_dynamic_vector_element->key = _last_key++;
...
void dynamic_vector_remove(t_dynamic_vector *dyn_vec, t_key key)
{
	...
	idx = dynamic_vector_find_key (dyn_vec, key);
	
	if (idx == 0 || idx > dyn_vec->num_elements)
	{
		return;
	}
	...
	free(dyn_vec->elements[idx].element);
