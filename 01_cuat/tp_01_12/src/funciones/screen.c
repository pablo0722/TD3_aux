 ...
 * 					FUNCIONES 					*
 ************************************************/
__FUNCTION__ static void screen_cursor( byte flags)
{
...
__FUNCTION__ void screen_reset()
{
	...
	screen_set_color(FG_RED | FG_GREEN | FG_BLUE | INTENSE);

	video_col = 0;
	video_row = 0;

	screen_cursor( video_flags);
}
...
__FUNCTION__ void screen_write_str( const char *string )
{
	while( *string != 0 )
	{
		screen_write_char( *string++, video_flags );
	}
}
...
__FUNCTION__ void screen_write_str_color( const char *string, byte flags )
{
	while( *string != 0 )
	{
		screen_write_char( *string++, flags );
	}
}
...
__FUNCTION__ void screen_write_str_nl_color( const char *string, byte flags )
{
	...
	screen_write_str_color( string, flags );
}
...
__FUNCTION__ void screen_write_byte( byte b )
{
	...
	for(i = 100; i >= 1; i /= 10)
	{
		...
		{
			is_cero = 0;
			screen_write_char( ((b / i) % 10) + '0', video_flags );
		}
	}

	if(is_cero)
	{
		screen_write_char( '0', video_flags );
	}
}
...
__FUNCTION__ void screen_write_word( word w )
{
	...
	for(i = 10000; i >= 1; i /= 10)
	{
		...
		{
			is_cero = 0;
			screen_write_char( ((w / i) % 10) + '0', video_flags );
		}
	}
...
__FUNCTION__ void screen_write_dword_color( dword dw, byte flags )
{
	...
	for(i = 1000000000; i >= 1; i /= 10)
	{
		...
		{
			is_cero = 0;
			screen_write_char( ((dw / i) % 10) + '0', flags );
		}
	}

	if(is_cero)
	{
		screen_write_char( '0', flags );
	}
}
...
{
	screen_write_dword_color( dw, video_flags );
}
...
__FUNCTION__ void screen_write_byte_hex( byte b )
{
	...
	screen_write_char( '0', video_flags );
	screen_write_char( 'x', video_flags );
	...
	for(i = 0x10; i >= 1; i /= 0x10)
	{
		if(b / i != 0)
		{
			...
			screen_write_char( character, video_flags );
		}
	}
...
__FUNCTION__ void screen_write_dword_hex_color( dword dw, byte flags )
{
	...
	for(i = 0x10000000; i >= 1; i /= 0x10)
	{
		if(dw / i != 0)
		{
			...
			screen_write_char( character, flags );
		}
	}
...
{
	screen_write_dword_hex_color( dw, video_flags );
}