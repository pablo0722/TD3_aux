...
extern byte __SHDW_KFUNC32_PHY;
extern byte __SHDW_KFUNC32_LIN;
extern byte __SHDW_KFUNC32_LMA;
extern byte __SHDW_KFUNC32_LEN;
...
__KERNEL__ static void _idt_init()
{
	_isr_load(&(idt_gates[IRQ_DE]), 	&_IrqDE, 	(dword) &CODE_SEL0, DPL_KERNEL, IDT_TYPE_TRAP);
	// _isr_load(&(idt_gates[IRQ1]), 		NULL, 0, 0);
	_isr_load(&(idt_gates[IRQ_NMI]), 	&_IrqNMI, 	(dword) &CODE_SEL0, DPL_KERNEL, IDT_TYPE_INT);
	_isr_load(&(idt_gates[IRQ_BP]), 	&_IrqBP, 	(dword) &CODE_SEL0, DPL_KERNEL, IDT_TYPE_TRAP);
	_isr_load(&(idt_gates[IRQ_OF]), 	&_IrqOF, 	(dword) &CODE_SEL0, DPL_KERNEL, IDT_TYPE_TRAP);
	_isr_load(&(idt_gates[IRQ_BR]), 	&_IrqBR, 	(dword) &CODE_SEL0, DPL_KERNEL, IDT_TYPE_TRAP);
	_isr_load(&(idt_gates[IRQ_UD]), 	&_IrqUD, 	(dword) &CODE_SEL0, DPL_KERNEL, IDT_TYPE_TRAP);
	_isr_load(&(idt_gates[IRQ_NM]), 	&_IrqNM, 	(dword) &CODE_SEL0, DPL_KERNEL, IDT_TYPE_TRAP);
	_isr_load(&(idt_gates[IRQ_DF]), 	&_IrqDF, 	(dword) &CODE_SEL0, DPL_KERNEL, IDT_TYPE_TRAP);
	// _isr_load(&(idt_gates[IRQ9]), 		NULL, 0, 0);
	_isr_load(&(idt_gates[IRQ_TS]), 	&_IrqTS, 	(dword) &CODE_SEL0, DPL_KERNEL, IDT_TYPE_TRAP);
	_isr_load(&(idt_gates[IRQ_NP]), 	&_IrqNP, 	(dword) &CODE_SEL0, DPL_KERNEL, IDT_TYPE_TRAP);
	_isr_load(&(idt_gates[IRQ_SS]), 	&_IrqSS, 	(dword) &CODE_SEL0, DPL_KERNEL, IDT_TYPE_TRAP);
	_isr_load(&(idt_gates[IRQ_GP]), 	&_IrqGP, 	(dword) &CODE_SEL0, DPL_KERNEL, IDT_TYPE_TRAP);
	_isr_load(&(idt_gates[IRQ_PF]), 	&_IrqPF, 	(dword) &CODE_SEL0, DPL_KERNEL, IDT_TYPE_TRAP);
	// _isr_load(&(idt_gates[IRQ15]), 		NULL, 0, 0);
	_isr_load(&(idt_gates[IRQ_MF]), 	&_IrqMF, 	(dword) &CODE_SEL0, DPL_KERNEL, IDT_TYPE_TRAP);
	_isr_load(&(idt_gates[IRQ_AC]), 	&_IrqAC, 	(dword) &CODE_SEL0, DPL_KERNEL, IDT_TYPE_TRAP);
	_isr_load(&(idt_gates[IRQ_MC]), 	&_IrqMC, 	(dword) &CODE_SEL0, DPL_KERNEL, IDT_TYPE_TRAP);
	_isr_load(&(idt_gates[IRQ_XF]), 	&_IrqXF, 	(dword) &CODE_SEL0, DPL_KERNEL, IDT_TYPE_TRAP);
	...
	_isr_load(&(idt_gates[IRQ_TM0]), 	&_IrqTM0, 	(dword) &CODE_SEL0, DPL_KERNEL, IDT_TYPE_INT);
	_isr_load(&(idt_gates[IRQ_KB]), 	&_IrqKB, 	(dword) &CODE_SEL0, DPL_KERNEL, IDT_TYPE_INT);
	_isr_load(&(idt_gates[IRQ_SC]), 	&_IrsSC, 	(dword) &CODE_SEL0, DPL_USER, 	IDT_TYPE_TRAP); // int 0x80
...
__KERNEL__ static void _pdt_init()
{
	...
	{
		_pagine(pdt_tarea0, pt_tarea0, (&__SHDW_ISR_PHY + offset), (&__SHDW_ISR_LIN + offset), PAGE_ATTR_KERNEL | PAGE_ATTR_R);
		_pagine(pdt_tarea1, pt_tarea1, (&__SHDW_ISR_PHY + offset), (&__SHDW_ISR_LIN + offset), PAGE_ATTR_KERNEL | PAGE_ATTR_R);
		_pagine(pdt_tarea2, pt_tarea2, (&__SHDW_ISR_PHY + offset), (&__SHDW_ISR_LIN + offset), PAGE_ATTR_KERNEL | PAGE_ATTR_R);
	}
	...
	{
		_pagine(pdt_tarea0, pt_tarea0, (&__SHDW_KERNEL32_PHY + offset), (&__SHDW_KERNEL32_LIN + offset), PAGE_ATTR_KERNEL | PAGE_ATTR_R);
		_pagine(pdt_tarea1, pt_tarea1, (&__SHDW_KERNEL32_PHY + offset), (&__SHDW_KERNEL32_LIN + offset), PAGE_ATTR_KERNEL | PAGE_ATTR_R);
		_pagine(pdt_tarea2, pt_tarea2, (&__SHDW_KERNEL32_PHY + offset), (&__SHDW_KERNEL32_LIN + offset), PAGE_ATTR_KERNEL | PAGE_ATTR_R);
	}
	...
	for(offset = 0x00000000; offset < (dword) &__SHDW_KFUNC32_LEN; offset += 0x1000)
	{
		_pagine(pdt_tarea0, pt_tarea0, (&__SHDW_KFUNC32_PHY + offset), (&__SHDW_KFUNC32_LIN + offset), PAGE_ATTR_KERNEL | PAGE_ATTR_R);
		_pagine(pdt_tarea1, pt_tarea1, (&__SHDW_KFUNC32_PHY + offset), (&__SHDW_KFUNC32_LIN + offset), PAGE_ATTR_KERNEL | PAGE_ATTR_R);
		_pagine(pdt_tarea2, pt_tarea2, (&__SHDW_KFUNC32_PHY + offset), (&__SHDW_KFUNC32_LIN + offset), PAGE_ATTR_KERNEL | PAGE_ATTR_R);
	}

	for(offset = 0x00000000; offset < (dword) &__SHDW_FUNC32_LEN; offset += 0x1000)
	{
		_pagine(pdt_tarea0, pt_tarea0, (&__SHDW_FUNC32_PHY + offset), (&__SHDW_FUNC32_LIN + offset), PAGE_ATTR_USER | PAGE_ATTR_R);
		_pagine(pdt_tarea1, pt_tarea1, (&__SHDW_FUNC32_PHY + offset), (&__SHDW_FUNC32_LIN + offset), PAGE_ATTR_USER | PAGE_ATTR_R);
		_pagine(pdt_tarea2, pt_tarea2, (&__SHDW_FUNC32_PHY + offset), (&__SHDW_FUNC32_LIN + offset), PAGE_ATTR_USER | PAGE_ATTR_R);
	}

	for(offset = 0x00000000; offset < (dword) &__TABLA_DIGITOS_LEN; offset += 0x1000)
	{
		_pagine(pdt_tarea0, pt_tarea0, (&__TABLA_DIGITOS_PHY + offset), (&__TABLA_DIGITOS_LIN + offset), PAGE_ATTR_USER | PAGE_ATTR_RW);
		_pagine(pdt_tarea1, pt_tarea1, (&__TABLA_DIGITOS_PHY + offset), (&__TABLA_DIGITOS_LIN + offset), PAGE_ATTR_USER | PAGE_ATTR_RW);
		_pagine(pdt_tarea2, pt_tarea2, (&__TABLA_DIGITOS_PHY + offset), (&__TABLA_DIGITOS_LIN + offset), PAGE_ATTR_USER | PAGE_ATTR_RW);
	}

	/*
	for(offset = 0x00000000; offset < (dword) &__KEYBOARD_BUFFER_LEN; offset += 0x1000)
	{
		_pagine(pdt_tarea1, pt_tarea1, (&__KEYBOARD_BUFFER_PHY + offset), (&__KEYBOARD_BUFFER_LIN + offset), PAGE_ATTR_USER | PAGE_ATTR_RW);
	}
	*/
...
__KERNEL__ static void _pdt_tarea0_init()
{
	...
	_pagine(pdt_tarea0, pt_tarea0, &__TAREA0_TEXT_PHY, 			&__TAREA_TEXT_LIN, 			PAGE_ATTR_USER 		| PAGE_ATTR_R);
	_pagine(pdt_tarea0, pt_tarea0, &__TAREA0_BSS_PHY, 			&__TAREA_BSS_LIN, 			PAGE_ATTR_USER 		| PAGE_ATTR_RW);
	_pagine(pdt_tarea0, pt_tarea0, &__TAREA0_DATA_PHY, 			&__TAREA_DATA_LIN, 			PAGE_ATTR_USER 		| PAGE_ATTR_RW);
	_pagine(pdt_tarea0, pt_tarea0, &__STACK_TAREA0_NIVEL0_PHY,  &__STACK_TAREA_NIVEL0_LIN, 	PAGE_ATTR_KERNEL 	| PAGE_ATTR_RW);
	_pagine(pdt_tarea0, pt_tarea0, &__STACK_TAREA0_NIVEL3_PHY,  &__STACK_TAREA_NIVEL3_LIN, 	PAGE_ATTR_USER 		| PAGE_ATTR_RW);
}
...
__KERNEL__ static void _pdt_tarea1_init()
{
	...
	_pagine(pdt_tarea1, pt_tarea1, &__TAREA1_TEXT_PHY, 			&__TAREA_TEXT_LIN, 			PAGE_ATTR_USER 		| PAGE_ATTR_R);
	_pagine(pdt_tarea1, pt_tarea1, &__TAREA1_BSS_PHY, 			&__TAREA_BSS_LIN, 			PAGE_ATTR_USER 		| PAGE_ATTR_RW);
	_pagine(pdt_tarea1, pt_tarea1, &__TAREA1_DATA_PHY, 			&__TAREA_DATA_LIN, 			PAGE_ATTR_USER 		| PAGE_ATTR_RW);
	_pagine(pdt_tarea1, pt_tarea1, &__STACK_TAREA1_NIVEL0_PHY,  &__STACK_TAREA_NIVEL0_LIN, 	PAGE_ATTR_KERNEL 	| PAGE_ATTR_RW);
	_pagine(pdt_tarea1, pt_tarea1, &__STACK_TAREA1_NIVEL3_PHY,  &__STACK_TAREA_NIVEL3_LIN, 	PAGE_ATTR_USER 		| PAGE_ATTR_RW);
}
...
__KERNEL__ static void _pdt_tarea2_init()
{
	...
	_pagine(pdt_tarea2, pt_tarea2, &__TAREA2_TEXT_PHY, 			&__TAREA_TEXT_LIN, 			PAGE_ATTR_USER 		| PAGE_ATTR_R);
	_pagine(pdt_tarea2, pt_tarea2, &__TAREA2_BSS_PHY, 			&__TAREA_BSS_LIN, 			PAGE_ATTR_USER 		| PAGE_ATTR_RW);
	_pagine(pdt_tarea2, pt_tarea2, &__TAREA2_DATA_PHY, 			&__TAREA_DATA_LIN, 			PAGE_ATTR_USER 		| PAGE_ATTR_RW);
	_pagine(pdt_tarea2, pt_tarea2, &__STACK_TAREA2_NIVEL0_PHY,  &__STACK_TAREA_NIVEL0_LIN, 	PAGE_ATTR_KERNEL 	| PAGE_ATTR_RW);
	_pagine(pdt_tarea2, pt_tarea2, &__STACK_TAREA2_NIVEL3_PHY,  &__STACK_TAREA_NIVEL3_LIN, 	PAGE_ATTR_USER 		| PAGE_ATTR_RW);
}